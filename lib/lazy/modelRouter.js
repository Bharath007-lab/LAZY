...
import { getActiveGateway } from './integrations.js'
...
export async function route({
  ...
  // BYOK gateway (OpenRouter / Merge Gateway) takes precedence when connected.
  const gw = await getActiveGateway()
  if (gw) {
    const t0 = Date.now()
    try {
      const client = new OpenAI({ apiKey: gw.apiKey, baseURL: gw.baseURL })
      const sys = json ? `${system}\n\nReturn ONLY valid minified JSON. No markdown, no commentary.` : system
      const resp = await client.chat.completions.create({
        model: gw.model,
        messages: [{ role: 'system', content: sys }, { role: 'user', content: user }],
        temperature: json ? 0.1 : 0.4, max_tokens: 1500,
      })
      const text = resp.choices?.[0]?.message?.content || ''
      const out = json ? parseJson(text) : null
      const latency = Date.now() - t0
      await logRequest({ id: randomUUID(), provider: gw.provider, model: gw.model, capability, success: true, latency_ms: latency, fallback_used: false, gateway: true, task_units: taskUnits, cost: Number((taskUnits * AI_COST_PER_UNIT).toFixed(4)), agent: meta.agent || null, user_id: meta.userId || null, createdAt: new Date() })
      return { text, json: out, model: gw.model, provider: gw.provider, fallback_used: false, latency_ms: latency }
    } catch (e) {
      await logRequest({ id: randomUUID(), provider: gw.provider, model: gw.model, capability, success: false, latency_ms: Date.now() - t0, gateway: true, error: String(e?.message || e).slice(0, 300), agent: meta.agent || null, createdAt: new Date() })
      // fall through to Emergent-managed models
    }
  }
