...
export async function listIntegrations() {
  ...
  return Object.entries(PROVIDERS).map(([id, meta]) => {
    ...
    return {
      ...
      primary: !!it.primary,
      masked_key: mask(it.key || it.secret || it.endpoint),
...
export async function saveIntegration(provider, data) {
  ...
  for (const f of PROVIDERS[provider].fields) {
    if (data[f] !== undefined && data[f] !== '') set[`integrations.${provider}.${f}`] = data[f]
  }
...
async function validate(provider, it) {
  try {
  ...
  try {
    ...
    for (const p of ['openrouter', 'merge']) {
      ...
      if (it && it.status === 'connected' && it.primary && it.key) {
        return { provider: p, apiKey: it.key, baseURL: PROVIDERS[p].base, model: it.model || PROVIDERS[p].default_model }
      }
    }
  } catch { /* ignore */ }
  return null
